...
const AIAgentEvaluator = () => {
  ...
  const WEBHOOK_POST = 'https://cyogiswara.app.n8n.cloud/webhook-test/upload-tugas';
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="relative group">
              ...
              <div className={`h-44 rounded-3xl border-2 border-dashed transition-all p-6 bg-slate-900/40 backdrop-blur-md flex flex-col items-center justify-center ${files.soal ? 'border-indigo-500 bg-indigo-500/5' : 'border-slate-800'}`}>
                <div className={`p-4 rounded-xl mb-3 ${files.soal ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-slate-800 text-slate-500'}`}>
                  {files.soal ? <CheckCircle2 size={20} /> : <FileText size={20} />}
                </div>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">PDF Soal & Rubrik</span>
              </div>
            </div>
            ...
            <div className="relative group">
              ...
              <div className={`h-44 rounded-3xl border-2 border-dashed transition-all p-6 bg-slate-900/40 backdrop-blur-md flex flex-col items-center justify-center ${files.jawaban ? 'border-emerald-500 bg-emerald-500/5' : 'border-slate-800'}`}>
                <div className={`p-4 rounded-xl mb-3 ${files.jawaban ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/20' : 'bg-slate-800 text-slate-500'}`}>
                  {files.jawaban ? <CheckCircle2 size={20} /> : <Archive size={20} />}
                </div>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">ZIP File Jawaban</span>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-3xl p-8 text-white relative overflow-hidden shadow-xl">
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all active:scale-[0.98] ${!files.soal || !files.jawaban ? 'bg-black/20 text-white/30 cursor-not-allowed' : 'bg-white text-indigo-900 shadow-lg hover:bg-indigo-50'}`}
            >
          ...
          <div className="bg-[#020617] border border-slate-800 rounded-3xl p-6 h-[200px] overflow-y-auto shadow-inner custom-scrollbar">
            ...
            <div className="space-y-2">
              {logs.length > 0 ? logs.map(log => (
                <div key={log.id} className="text-[11px] text-slate-400 flex gap-2 animate-in fade-in slide-in-from-left-1">
                  <span className="text-emerald-500">›</span> {log.msg}
                </div>
              )) : (
                <div className="text-[11px] text-slate-600 italic">Menunggu aktivitas...</div>
              )}
            </div>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-950/50 text-slate-500 uppercase text-[9px] tracking-widest">
                  <tr>
                    <th className="p-4 font-bold border-b border-slate-800">Mahasiswa (Folder)</th>
                    <th className="p-4 text-center font-bold border-b border-slate-800">Skor</th>
                    <th className="p-4 font-bold border-b border-slate-800">Mata Kuliah</th>
                    <th className="p-4 font-bold border-b border-slate-800">Feedback AI</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800">
                  {results.length > 0 ? results.map((res, i) => (
                    <tr key={i} className="hover:bg-slate-800/30 transition-colors group">
                      <td className="p-4 text-slate-200 font-medium">{res["NIM / File"] || "N/A"}</td>
                      <td className="p-4 text-center text-emerald-400 font-bold text-sm">{res["Score"] ?? 0}</td>
                      <td className="p-4 text-slate-400">{res["Matkul"] || "N/A"}</td>
                      <td className="p-4 max-w-xs text-slate-500 italic text-[11px] leading-relaxed group-hover:text-slate-300 transition-colors">
                        {res["Feedback"] || "-"}
                      </td>
                    </tr>
                  )) : (
